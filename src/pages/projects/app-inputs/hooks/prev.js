...
const useForm = ({ init, validate }) => {
  ...
  const handleChange = (e) => {
    ...
    // get updated values from the NEW state
    const values = mapStateToKeys(oldState, "value");

    console.log("Value after update: handleChange: ", value);
    ...
    if (typeof validate === "function") {
      ...
      errors = mapStateToKeys(oldState, "error");
    } else {
      throw new Error("Validate must be boolean or function");
    }

    // update error for the current field
    if (oldState[key].touched && errors[key]) {
      oldState[key].error = errors[key];
    } else {
      oldState[key].error = "";
